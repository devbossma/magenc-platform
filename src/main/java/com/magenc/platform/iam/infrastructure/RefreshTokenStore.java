// Exists because refresh tokens must be opaque, hashed, single-use, and revocable: JWTs alone cannot provide all four.
...
public class RefreshTokenStore {
    ...
    public RefreshTokenStore(JdbcTemplate jdbc) { this.jdbc = jdbc; }
    ...
    public void store(String tokenValue, UUID userId, String tenantSlug, Instant expiresAt) {
        ...
        jdbc.update("""
                ...
                VALUES (?, ?, ?, ?, ?)
                """, hash, userId, tenantSlug,
                java.sql.Timestamp.from(expiresAt), java.sql.Timestamp.from(Instant.now()));
    }
    ...
    public void revoke(String tokenValue) {
        jdbc.update("DELETE FROM admin.refresh_token WHERE token_hash = ?", sha256(tokenValue));
    }
